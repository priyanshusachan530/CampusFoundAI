...
export default function AdminFoundItems() {
  ...
  const loadData = () => {
    const all = getItems();
    setItems(all.filter((i) => i.type === "found"));
  };
  ...
  const handleConfirmDelete = () => {
    ...
    loadData();
    addToast("Found report deleted successfully.", "success");
  };
  ...
  const handleSaveLocker = (id) => {
    updateItem(id, { custodyLocation: lockerValue });
    setEditingLockerId(null);
    loadData();
    addToast(`Custody locker location updated.`, "success");
  };

  const handleExportCSV = () => {
    const rows = [
      ["ID", "Item Name", "Category", "Location Found", "Date", "Status", "Custody Locker"]
    ];
    items.forEach((item) => {
      rows.push([
        item.id,
        ...
        item.status,
        item.custodyLocation || "Central Locker #01"
      ]);
    });
    ...
    link.setAttribute("download", `campus-found-items-${new Date().toISOString().split("T")[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    addToast("Exported Found Items CSV successfully.", "success");
  };
  ...
  const filtered = items.filter((item) => {
    ...
    const matchesLoc = location === "all" || (item.locationFound || item.location) === location;
    const matchesStatus = status === "all" || item.status === status;
    return matchesSearch && matchesCat && matchesLoc && matchesStatus;
  });

  return (
    <AdminLayout activeTab="found-items">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
              ...
              Found Items Custody Inventory ({items.length})
            </h1>
            <p className="text-xs text-slate-500 mt-1">
              All physical property turned in to campus security, department offices, and desks.
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 shadow-xs grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
          ...
          <div>
            ...
            >
              ...
              <option value="ACTIVE">FOUND (IN CUSTODY)</option>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs overflow-hidden">
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 text-slate-500 font-bold uppercase tracking-wider text-[11px]">
                <tr>
                  ...
                  <th className="py-3 px-4">Location Found</th>
                  <th className="py-3 px-4">Custody Locker</th>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/40">
                      ...
                      <td className="py-3.5 px-4">
                        ...
                        <div className="text-[11px] text-slate-400">
                          {item.category} • {item.color} {item.brand ? `• ${item.brand}` : ""}
                        </div>
                      </td>
                        ...
                        {editingLockerId === item.id ? (
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              placeholder="Locker #..."
                              className="w-28 text-[11px] px-2 py-1 rounded-md border border-indigo-400 bg-white dark:bg-slate-900"
                              autoFocus
                            />
                            <button
                              onClick={() => handleSaveLocker(item.id)}
                          ...
                          <div
                            onClick={() => {
                              setEditingLockerId(item.id);
                              setLockerValue(item.custodyLocation || "Locker #A12");
                            }}
                            className="cursor-pointer text-slate-600 dark:text-slate-300 hover:text-indigo-600 flex items-center gap-1 font-mono text-[11px]"
                            title="Click to edit custody locker"
                          >
                            <Lock className="w-3 h-3 text-slate-400" />
                            <span>{item.custodyLocation || "Locker #A12"}</span>
                            <Edit2 className="w-2.5 h-2.5 text-slate-400 opacity-60" />
                          </div>
                        )}
                      </td>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-10 text-xs text-slate-400">
              No found items match the selected filter parameters.
            </div>
          )}
        </div>
